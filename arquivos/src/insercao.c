...
*/
void inserir_best_fit(FILE *fbin, JOGADOR *novo, CABECALHO *cab, LISTA *reg) {
    ...
    if (cab->nroRegRem == 0) {
        ...
        inserir_lista(reg, novo, cab->proxByteOffset);
    ...
    if (novo->tamanhoRegistro < atual->tamanhoRegistro) {
        ...
        // inserir na lista ordenada
        inserir_lista(reg, novo, cab->topo);
        ...
        while (1) {
            ...
            if (novo->tamanhoRegistro > atual->tamanhoRegistro &&
                ...
                // inserir na lista ordenada
                inserir_lista(reg, novo, byteoffset_prox);
...
void INSERT_INTO(char *nomeArquivoBinario, char *nomeArquivoIndice) {
    ...
    while (n--) {
        ...
        inserir_best_fit(fbin, novo, cab, reg);
        cab->nroRegArq++;
